...
//      jdbcTemplate.execute("SET GLOBAL event_scheduler = ON;"); comentado, pois no Google Cloud SQL não é possível habilitar o event scheduler desta forma e ele já vem habilitado por padrão.
   ...
   private void loadUsuario() {
      if (usuarioRepository.count() == 0) {
         usuarioRepository.save(Usuario.builder().role(ADMIN).nome("UsuarioADM").email("adm@reportai.com").senha("$2a$10$8hgBJE/yHhE.zUXD1TecoO9.lY77x0UxLnXR6S6JhX0Mo/EOE234C").cpf("160.410.930-09").segundoFator(false).isDeleted(false).build());
         usuarioRepository.save(Usuario.builder().role(USUARIO).nome("João Silva").email("user@reportai.com").senha("$2a$10$4UGjigwF75Y8BKyeRtnDM.miLFkRi8.AcJ1aFlcSD6N11EN1LpYTa").cpf("839.925.170-47").segundoFator(false).isDeleted(false).build());
         usuarioRepository.save(Usuario.builder().role(USUARIO).nome("Maria Marques").email("maria@marques.com").senha("$2a$10$e1uB2k4cvYzDw0ZVjuegMOeEIiQSUgug6bu0OcxELTlfS4EMgc.Zi").cpf("532.162.220-55").segundoFator(false).isDeleted(false).build());
         usuarioRepository.save(Usuario.builder().role(USUARIO).nome("Joaquim Silva").email("joaquim@silva.com").senha("$2a$10$raw1tiGfMm6jYPeVRtwwh.sGzOatV3dDRoxnY7BOY4fmNURQF1GEu").cpf("540.924.870-88").segundoFator(false).isDeleted(false).build());
         usuarioRepository.save(Usuario.builder().role(USUARIO).nome("Márcio Mendes").email("marcio@mendes.com").senha("$2a$10$o/7apx/WgfcuRjGza72khelJDR0dOxwbaiduZjHveAV1pOBBJkNIq").cpf("745.704.400-02").segundoFator(false).isDeleted(false).build());
      }
   }
   ...
   private void loadRegistro() {
      if (registroRepository.count() == 0) {
         ...
         jdbcTemplate.execute("UPDATE registro SET dt_criacao = '2025-06-20 11:05:46' WHERE id = 4;");
         ...
         jdbcTemplate.execute("UPDATE registro SET dt_criacao = '2025-07-05 09:23:10' WHERE id = 9;");
